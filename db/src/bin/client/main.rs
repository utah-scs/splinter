    ...
    T: PacketTx + PacketRx + Display + Clone + 'static,
{
    ...
    // Instance of YCSB workload to drive client, if any.
    ycsb_bench: Option<RefCell<ycsb::Ycsb>>,
    ...
    T: PacketTx + PacketRx + Display + Clone + 'static,
{
    ...
    fn new(config: &config::ClientConfig, port: T) -> RequestGenerator<T> {
        ...
        let bench = if config.use_ycsb {
                Some(RefCell::new(
                        ycsb::Ycsb::new(config.key_len, config.value_len,
                                        config.n_keys, config.put_pct, config.skew)))
            } else {
                None
            };
    ...
    fn generate_request(&self) {
        let request = if let Some(ref ycsb_bench) = self.ycsb_bench {
                ycsb_bench.borrow_mut().abc(|key| {
                        self.create_get_request(1, 1, key)
                    },
                    |key, _value| {
                        self.create_get_request(1, 1, key)
                    })
                ...
                if self.use_invoke {
                    ...
                    self.create_invoke_request(1, name.len(),
                                               payload.len() - name.len(), payload.as_slice())
                } else {
