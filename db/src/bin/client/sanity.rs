...
impl SanitySend {
    ...
    fn new(config: &config::ClientConfig, port: Arc<RwLock<CacheAligned<PortQueue>>>) -> SanitySend {
        SanitySend {
    ...
    T: PacketTx + PacketRx + Display + Clone + 'static,
{
    ...
    fn new(port: Arc<RwLock<T>>) -> SanityRecv<T> {
        SanityRecv {
...
fn setup_recv<T, S>(ports: Vec<Arc<RwLock<T>>>, scheduler: &mut S)
where
    T: PacketTx + PacketRx + Display + Clone + 'static,
    S: Scheduler + Sized,
{
